...
export interface Cell {
  ...
  alive: boolean;
  // 질병(감염): 독성이 높을수록 감기처럼 병에 걸린다. 병중엔 에너지 소모가 커지고,
  // 시간이 지나면 회복. 렌더러가 보라색으로 표시한다.
  sick: boolean;
  // 세대: 초기 세포=0, 분열로 태어난 자식=부모+1. 개체 정보 표시용.
  gen: number;
  // 개체 편차(체질): birth 시점에 고정된 난수 배율(≈1). 분열 문턱에 곱해져
  // 같은 종이라도 분열/사망 타이밍이 개체마다 달라진다(한꺼번에 죽고 분열하는 동기화 완화).
  jitter: number;
