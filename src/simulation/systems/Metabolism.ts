...
export function runMetabolism(world: World, dt: number): void {
  ...
  for (let i = 0; i < cells.length; i++) {
    ...
    // 4) 대사열 발생
    env.add('heat', (def.energyFromIntake * satisfaction + def.energyFromScavenge) * dt * 0.15);

    // 5) 에너지 수지 (열 과잉 시 기초대사 증가)
    const heatPenalty = 1 + Math.max(0, env.resources.heat - 600) / 1200;
    c.energy += def.energyFromIntake * satisfaction * dt + scavengeEnergy;
    ...
    const o2Need = def.upkeep * cfg.respirationRate * dt;
    if (o2Need > 0) {
      const breathe = env.consume('oxygen', o2Need); // 확보 비율 0..1
      env.add('co2', o2Need * breathe * cfg.respirationCo2Ratio);
      if (breathe < 1) {
        c.energy -= def.upkeep * (1 - breathe) * cfg.suffocationPenalty * dt;
      }
    }
