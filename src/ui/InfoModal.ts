...
import type { ChoiceDef, Effect } from '../rules/types';
import { describeEffect } from './FeedbackLayer';
...
const ROLE_TEXT: Record<SpeciesId, string> = {
  ...
  consumer:
    '광합성 세포를 뜯어먹는 초식자. 광합성 개체수를 조절하지만, 너무 많아지면 광합성을 과도하게 먹어 산소 생산이 무너지고 스스로도 질식한다.',
  predator:
    '소비 세포를 사냥하는 육식자. 소비 세포의 과잉 번식을 억제하지만, 포식자가 너무 많으면 먹이가 전멸해 함께 굶는다.',
  decomposer:
    '유기물(사체)을 먹고 독성을 정화하는 청소부. 사체가 쌓여 독성이 폭증할 때 생태계를 지키는 마지막 방어선.',
};
...
/** intake/output 맵을 "유기물 3/s · 산소 1.5/s" 형태로 */
...
/** 이 Effect가 특정 종에 관련된 것인지 */
function effectTouchesSpecies(e: Effect, id: SpeciesId): boolean {
  return 'species' in e && e.species === id;
}

/**
 * 세포 도움말 모달. 하단 세포 바를 탭하면 열린다(게임은 일시정지).
 * 역할 설명 + 현재(변경 반영된) 대사 수치 + 적용된 진화(선택) 이력을 보여준다.
 */
export class InfoModal {
  ...
  show(id: SpeciesId, world: World, appliedChoices: ChoiceDef[]): void {
    const def = world.species[id]; // 런타임 명세(선택 Effect 반영됨)
    ...
    // 이 종에 영향을 준 선택 이력
    const evolutions = appliedChoices
      .map((choice) => {
        const related = choice.effects.filter((e) => effectTouchesSpecies(e, id));
        if (related.length === 0) return null;
        return { title: choice.title, lines: related.map(describeEffect) };
      })
      .filter((x): x is { title: string; lines: string[] } => x !== null);

    const evoHtml =
      evolutions.length === 0
        ? `<div class="modal-empty">아직 적용된 진화가 없습니다.</div>`
        : evolutions
            .map(
              (ev) => `
              <div class="evo-item">
                <div class="evo-title">${ev.title}</div>
                ${ev.lines.map((l) => `<div class="evo-line">${l}</div>`).join('')}
              </div>`,
            )
            .join('');
    ...
    this.el.innerHTML = `
      <div class="modal-card">
        ...
        <div class="modal-section">현재 대사</div>
        ...
        <div class="modal-section">적용된 진화</div>
        ${evoHtml}
      </div>`;
