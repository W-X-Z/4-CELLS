...
const ROLE_TEXT: Record<SpeciesId, string> = {
  photosynth:
    'CO₂→산소를 만드는 생태계의 엔진. 호흡하지 않고 광합성으로만 산다. 무너지면 산소가 말라 모두 질식.',
  consumer:
    '광합성 세포를 뜯어먹는 초식자. 너무 늘면 광합성을 고갈시켜 산소가 무너지고 자신도 질식한다.',
  predator:
    '소비 세포를 사냥하는 육식자. 소비 세포의 과증식을 억제하지만, 먹이가 마르면 함께 굶는다.',
  decomposer:
    '시체를 치우는 청소부. 시체를 없애야 독성이 잡힌다 — 생태계의 마지막 방어선.',
};
